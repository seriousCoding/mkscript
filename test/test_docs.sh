...
assert_file_contains "$repo_root/README.md" 'mkscript --template docker' 'README should include a Dockerfile template example'
...
assert_file_contains "$repo_root/INSTALL.md" 'mkscript --template docker-compose' 'INSTALL should include a Docker Compose example'
assert_file_contains "$repo_root/INSTALL.md" 'mkscript --template k8s-deployment deployment' 'INSTALL should include a Kubernetes example'
...
assert_file_contains "$repo_root/mkscript.1" 'template name is an alias for' 'man page should document the Dockerfile alias'
